...
func NewFs(ctx context.Context, name, root string, m configmap.Mapper) (outFs fs.Fs, err error) {
	...
	if foundArchive != nil {
		...
		// strip the archive path off remote
		fs.Debugf(nil, "remote = %q", remote)
		fs.Debugf(nil, "remote = %q", remote)
		...
		foundArchive.prefix = ""
		fs.Debugf(nil, "Found archive %#v", foundArchive)
...
func (f *Fs) FIXMEListR(ctx context.Context, dir string, callback fs.ListRCallback) (err error) {
	// defer log.Trace(f, "dir=%q, callback=%v", dir, callback)("err=%v", &err)
	return f.f.Features().ListR(ctx, dir, func(entries fs.DirEntries) error {
